...
public interface CollectorRepository extends JpaRepository<Collector, Integer> {
	...
	@Query(value="select kc.*, "
			...
			+ " from kwc_collector kc, kwc_group kg, kwc_site ks"
	...
	@Query(value="select `pk`, `status` from kwc_collector", nativeQuery = true)
	...
	@Query(value="select * from kwc.kwc_collector where site IN("
